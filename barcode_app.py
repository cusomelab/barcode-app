...
# ══════════════════════════════════════════════════════
with tab6:
    st.header('🏷️ 한글표시사항 라벨 생성')
    st.caption('의류 케어라벨을 작성하고 이미지로 다운로드합니다')

    # ── 케어 심볼 정의 (텍스트 기반) ─────────────────
    CARE_SYMBOLS = {
        '물세탁': {
            '95℃ 세탁':        ('🫧', '물 온도 95℃로 세탁 가능, 삶을 수 있음'),
            '60℃ 세탁':        ('🫧', '물 온도 60℃로 세탁 가능'),
            '40℃ 세탁':        ('🫧', '물 온도 40℃로 세탁 가능'),
            '약 40℃ 세탁':     ('🫧', '물 온도 40℃로 약하게 세탁'),
            '30℃ 중성 손세탁': ('🤲', '30℃ 미지근한 물에 중성세제로 손세탁'),
            '물세탁 금지':      ('🚫', '물세탁 불가'),
        },
        '표백': {
            '염소표백 가능':    ('✅', '염소계 표백제 사용 가능'),
            '산소표백 가능':    ('✅', '산소계 표백제만 사용 가능'),
            '표백 금지':        ('🚫', '표백제 사용 금지'),
        },
        '다림질': {
            '180~210℃ 다림질': ('♨️', '180~210℃로 다림질 가능 (고온)'),
            '140~160℃ 다림질': ('♨️', '140~160℃로 다림질 가능 (중온)'),
            '80~120℃ 다림질':  ('♨️', '80~120℃로 다림질 가능 (저온)'),
            '다림질 금지':      ('🚫', '다림질 금지'),
        },
        '드라이클리닝': {
            '드라이클리닝 가능':   ('👔', '드라이클리닝 가능'),
            '석유계 드라이클리닝': ('👔', '석유계 용제로 드라이클리닝'),
            '드라이클리닝 금지':   ('🚫', '드라이클리닝 금지'),
        },
        '건조': {
            '햇빛 건조':    ('☀️', '햇빛에 뉘어서 건조'),
            '그늘 건조':    ('🌤️', '그늘에서 건조'),
            '뉘어서 건조':  ('📐', '평평하게 뉘어서 건조'),
            '기계건조 가능':('🔄', '텀블드라이어 사용 가능'),
            '기계건조 금지':('🚫', '텀블드라이어 사용 금지'),
            '짜기 금지':    ('🚫', '비틀어 짜지 마세요'),
        },
    }
    ...
    with col_form:
        ...
        selected_symbols = []
        for category, symbols in CARE_SYMBOLS.items():
            with st.expander(f'▸ {category}', expanded=(category == '물세탁')):
    ...
    with col_preview:
        ...
        if st.button('🖼️ 라벨 이미지 생성', type='primary', key='hl_gen'):
            try:
                ...
                # ── 라벨 크기: 5cm x 7cm @ 150dpi = 295 x 413px
                W, H = 295, 413
                MARGIN = 16
                ...
                try:
                    ...
                    font_reg  = ImageFont.truetype(FONT_REG_PATH, 10)
                    font_title= ImageFont.truetype(FONT_PATH, 14)
                    font_small= ImageFont.truetype(FONT_REG_PATH, 8)
                except:
                ...
                title = '한글표시사항'
                tw = draw.textlength(title, font=font_title)
                draw.text(((W - tw) / 2, y), title, font=font_title, fill='black')
                y += 20
                draw.line([MARGIN, y, W-MARGIN, y], fill='black', width=2)
                y += 6

                # 항목 그리기 함수
                def draw_row(label, value, y, font_l=font_bold, font_v=font_reg, sep=True):
                    lw = 72
                    draw.text((MARGIN+4, y), label, font=font_l, fill='black')
                    # 값 줄바꿈 처리
                    lines = value.split('\n') if '\n' in value else textwrap.wrap(value, width=22) or [value]
                    for i, line in enumerate(lines):
                        draw.text((MARGIN+4+lw, y + i*12), line, font=font_v, fill='black')
                    row_h = max(16, len(lines)*12 + 4)
                    if sep:
                        draw.line([MARGIN, y+row_h, W-MARGIN, y+row_h], fill='#aaaaaa', width=1)
                    return y + row_h + 2

                y = draw_row('품    명', product_name or '미입력', y)
                y = draw_row('호    칭', size, y)
                y = draw_row('섬유의 조성', composition, y)
                y = draw_row('제조국명', FIXED['country'], y)
                y = draw_row('제조연월', FIXED['date'], y)
                y = draw_row('수입자명', FIXED['importer'], y)
                y = draw_row('주소 및\n전화번호', f"{FIXED['address']}\n{FIXED['phone']}", y)

                # 취급 주의사항
                y += 2
                draw.text((MARGIN+4, y), '취급상 주의사항', font=font_bold, fill='black')
                y += 14

                # 심볼 아이콘 줄
                icon_x = MARGIN + 4
                for _, icon, _ in selected_symbols:
                    draw.text((icon_x, y), icon, font=font_reg, fill='black')
                    icon_x += 22
                    if icon_x > W - MARGIN - 22:
                        icon_x = MARGIN + 4
                        y += 16
                y += 18

                # 심볼 텍스트 설명
                for sym_name, _, desc in selected_symbols:
                    lines = textwrap.wrap(f'• {desc}', width=38)
                    for line in lines:
                        if y < H - MARGIN - 8:
                            draw.text((MARGIN+4, y), line, font=font_small, fill='#333333')
                            y += 10

                # 추가 주의사항
                if custom_caution:
                    for line_raw in custom_caution.split('\n'):
                        if line_raw.strip():
                            for line in textwrap.wrap(f'• {line_raw.strip()}', width=38):
                                if y < H - MARGIN - 8:
                                    draw.text((MARGIN+4, y), line, font=font_small, fill='#333333')
                                    y += 10
                ...
                st.image(img, caption='라벨 미리보기', use_container_width=True)
            ...
            **한글표시사항** 미리보기
            ...
            | 항목 | 내용 |
            |------|------|
            | 품명 | {product_name or '미입력'} |
            | 호칭 | {size} |
            | 섬유 조성 | {composition.replace(chr(10), ', ')} |
            | 제조국 | {FIXED['country']} |
            | 제조연월 | {FIXED['date']} |
            | 수입자 | {FIXED['importer']} |
            | 주소 | {FIXED['address']} |
            ...
            """)
            if selected_symbols:
                st.markdown('**선택된 취급 주의사항:**')
                for name, icon, desc in selected_symbols:
